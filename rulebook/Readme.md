...
**请各位玩家根据 KP（Veda） 的指示，在本周六（2017-08-19 14:00 PM GMT+8）到达 YY CAC动漫社综合频道（67715380） 圣杯战争-说明会中，听取说明并等候其进一步安排**
...
#### 2.凭依
凭依即英灵会附着于玩家身上，而非Master-Servant分离的模式。凭依动作消耗20MP，每维持6小时消耗10MP。
...
#### 5.令咒机制
每人初始持有3个令咒：
令咒以使用命令的方式，以达到完成本来从者难以达到的事情，其使用方法多种多样。令咒可以影响英灵的所在位置、魔力、身体状态和宝具等。  
一个令咒曾被用于如下用途：
...
**回避**：攻击会和防守方的敏捷进行对抗，对抗每成功一级则对防守方多1HP伤害。  
**反击**：攻击会和防守方选择的反击类型（力量，迅捷，技巧）进行对抗，防守方固定受到1HP伤害。对抗每成功一级追加1HP伤害，失败则进攻方受到成功级别差的HP伤害。  
**物理防御**：攻击会和防守方的耐久进行对抗，进攻方对抗同级和成功无论级数差距均只造成1HP伤害，成功级数差为2级以上时，防守方会受到崩防效果，跳过自己下一个行动回合。  
**魔力防御**：攻击会和防守方的魔力进行对抗，每次使用固定消耗10MP。魔力防御在对抗近战攻击的时候需要添加一个惩罚骰。进攻方对抗同级和成功无论级数差距均只造成1HP伤害。  
